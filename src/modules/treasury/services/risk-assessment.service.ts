...
export class RiskAssessmentService {
  ...
  async create(riskAssessment: Partial<RiskAssessment>, userId: string): Promise<RiskAssessment> {
  ...
  async generateRiskAssessment(treasuryId: string, userId: string): Promise<RiskAssessment> {
    ...
    const overallScore = (marketRiskScore + counterpartyRiskScore + liquidityRiskScore) / 3;
    ...
    const riskAssessment: Partial<RiskAssessment> = {
      ...
      recommendations: this.generateRecommendations(assets, portfolioVolatility),
    };
    
    return this.create(riskAssessment, userId);
  }

  private generateRecommendations(assets: any[], portfolioVolatility: number): string[] {
    ...
    /*
    if (assets.length < 3) {
      recommendations.push('Consider diversifying your portfolio with more assets to reduce concentration risk.');
    }
    
    if (portfolioVolatility > 0.2) {
      recommendations.push('Your portfolio has high volatility. Consider adding stable assets to balance risk.');
    }
    
    if (assets.some(asset => asset.type === 'crypto' && asset.currentValue > 0)) {
      recommendations.push('Cryptocurrency assets can be highly volatile. Monitor market conditions regularly.');
    }
    
    // Add default recommendation if none were generated
    if (recommendations.length === 0) {
      recommendations.push('Your treasury appears to be well-balanced. Continue monitoring market conditions.');
    }
      */
  ...
  async update(id: string, riskAssessment: Partial<RiskAssessment>, userId: string): Promise<RiskAssessment> {
    const existingRiskAssessment = await this.riskAssessmentRepository.findById(id);
    const updatedRiskAssessment = await this.riskAssessmentRepository.update(id, riskAssessment);
    
    // Update the treasury's risk score if overall score changed
    if (riskAssessment.overallScore && existingRiskAssessment.overallScore !== riskAssessment.overallScore) {
